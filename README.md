...
The lower bound can be determined to be greater than half of the upper bound, or simply the `upperBound / 3`. This is because $2^{\frac{\log_2(n)}{3}} \leq \sqrt{n} \leq 2^{\frac{\log_2(n) + 2}{3}}$.
Given that we are performing binary search on a space that scales linearly with the size of the square root, giving a time complexity of $O(\frac{1}{2}\log_2(n))$
